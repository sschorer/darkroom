...
- **`make dev`, not `pnpm tauri dev`, on Linux/NVIDIA.** WebKitGTK's dmabuf renderer deadlocks the NVIDIA driver when the dev webview tears down: a thread wedges in `exit_mmap` unmapping GPU memory as an unkillable D-state task, no signal touches it, the frozen window can't be closed, and only a reboot recovers. The `dev` target sets `WEBKIT_DISABLE_DMABUF_RENDERER=1` to avoid it; running `tauri dev` bare skips that and reintroduces the hang.
