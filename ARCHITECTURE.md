...
**Status:** accepted
**Context:** The engine holds several GB of VRAM; an instance left running after the app goes away makes the next launch fail with an OOM the user cannot explain (§8.3, RISK-7, QS-6). §8.3 names three exit paths that must all reclaim it: normal quit (`RunEvent::ExitRequested`), the updater swapping the exe (`on_before_exit`, where NSIS terminates us without asking — #40), and the two no handler can cover, a `SIGKILL` of the app and a hard crash. The last pair is the hard one: by the time the app runs again, the only thing that survived is whatever we wrote to disk, and any handle to the child is long gone. Killing "the engine" from disk means killing by PID — and PIDs are recycled, so a bare PID read back at boot might now belong to an unrelated process that would be an innocent casualty.
**Decision:** `spawn` writes the engine's identity — `(pid, start_time)` — to `engine.pid` at the appdata root; `sidecar::reclaim_stale` reads it and kills the process only if one still lives at that PID whose start time matches *and* whose command line runs our `main.py`. One function serves the two live-app exits — boot (before this session spawns its own engine) and `ExitRequested` — both driven by the file rather than a retained child handle, so the same code reclaims a crashed prior run and a clean quit. Writing the record is **fatal if it fails**: teardown reads this file and nothing else, so an engine whose identity we could not persist is one no path could ever reclaim — `spawn` therefore stops the just-started child and returns an error rather than run an engine it can't account for (an unreadable start time is as disqualifying as an unwritable file: a record without it matches no live process). The record is removed once it has done its job — the engine was killed, was already gone, or the PID now belongs to a stranger — but is **kept if a kill we attempted fails**, so the next boot still has a pointer to a process that is still holding the GPU. `engine.pid` is a **sibling** of `engine/`, like `logs/` and `.uv`, so a reprovision that wipes the install can't strand the pointer to a process it left running. Reading and deleting the file are best-effort — a teardown that can't read or delete it must still let the app exit.
**Rationale:** `(pid, start_time)` is the standard defence against PID reuse — a recycled number always belongs to a later process with a different start second, so a match names the one process we recorded and no other. The command-line check is a required second guard, not a fallback: a process we cannot confirm is running our `main.py` — whose absolute appdata path appears in no unrelated process — is one we must not kill, so a hidden or empty command line *rejects* the reclaim rather than trusting the start time alone. In practice a live engine we spawned is our own process, whose command line the OS always lets us read; an unreadable one is not the engine, so requiring the match strands no real reclaim while closing the door on killing an innocent process that reused the PID in the same second. Persisting the identity is fatal-on-failure because this ticket exists to stop a GPU leak: an unwritable appdata directory is a broken install (it also breaks logs, models, and the db), and failing loudly beats running an engine we've quietly lost the ability to reclaim. Reclaiming through the file rather than a `CommandChild` means the crash path and the clean-exit path are one code path, and neither depends on a handle a crash would have dropped — the crash path is precisely the one that can't keep state in memory. `sysinfo` (only its `system` feature) is the one cross-platform way to read a process's start time and command line and to kill it (`SIGKILL` / `TerminateProcess`); a hard kill is right here because the target is an orphan past any graceful shutdown, and freeing the GPU *now* is the whole point.
**Consequences:** `engine.pid` joins the appdata contract in §7.2. `sysinfo` joins the dependency set. `spawn` gains a failure mode — `SpawnError::Pid` — that a caller must surface like any other (§8.6); it stops the child it started before returning. Reclaim *requires* a single running instance — it cannot tell our leaked engine from one a second Darkroom legitimately started — and that requirement is now enforced, not merely assumed: **ADR-017** turns a second launch away before it can run this reclaim. The whole appdata tree already assumed single-instance; ADR-017 makes the assumption load-bearing code has a right to. `spawn`, `pump`, and `wait_until_healthy` are now wired into the running lifecycle by `commands::start_engine`, and the boot and `ExitRequested` reclaim hooks are live. `on_before_exit` is deliberately out of scope until the updater exists; it will call the same `reclaim_stale`.
**Alternatives:** Store only the PID (rejected: kills whatever innocent process inherited a recycled number); best-effort identity persistence like the log pump (rejected: a missing log is a lost diagnostic, but a missing PID record is an unreclaimable GPU leak — the exact failure this ADR exists to prevent — so the two cannot share the log pump's stance); clear the record even when the kill failed (rejected: it discards the only pointer to a process still holding the GPU, so the next boot could not retry); trust the start time when the command line is hidden (rejected: it would kill a process we cannot confirm is ours, and a real engine's command line is always readable, so the fallback buys nothing and risks an innocent casualty); keep the `CommandChild` in Tauri state and kill through it (rejected: covers the clean quit but not the crash or `SIGKILL` — the exits that matter most — since the handle dies with the process that held it, so a second disk-based path would be needed anyway; one path is simpler); rely on the shell plugin killing its children on drop (rejected: it makes no such guarantee, which is why §8.3 exists); a lock/`flock` on a file (rejected: tells us an instance *was* running but neither which process nor how to end it); match the process by executable path (rejected: a venv's `python` is a symlink and the OS reports the resolved target, so the path wouldn't match what we spawned — the command-line `main.py` is stable where the exe is not).

### ADR-017 — Enforce a single running instance

**Status:** accepted
**Context:** ADR-016's engine reclaim is correct only for one running Darkroom. It kills the process named by `engine.pid` when a live process at that PID matches the recorded start time *and* runs our `main.py` — checks designed to spare an innocent process that inherited a recycled PID. But a *second* legitimate Darkroom fails all of those as a defence: its engine genuinely is our `main.py`, started at the recorded second, holding the recorded PID. So a second instance's boot-time `reclaim_stale` would confirm the first instance's live engine as a stale leak and `SIGKILL` it mid-generation. ADR-016 called single-instance "a separate concern"; wiring `spawn`/`reclaim_stale` into the real lifecycle (#39) makes it a prerequisite of correctness, not a nicety. The whole appdata tree — one `engine.pid`, one `logs/`, one SQLite db later — already assumes one writer.
**Decision:** Register `tauri-plugin-single-instance` as the **first** plugin, before `tauri-plugin-shell` and before the builder's own `setup`. On a second launch the plugin detects the primary (a platform-native lock — a socket on Linux, a named object on Windows, a Mach port on macOS), hands the primary the new args via the callback, and exits the second process *inside the plugin's own setup* — before our `setup` closure could run `reclaim_engine`. The callback focuses the existing window, so a user who relaunches gets the running app surfaced rather than silence. No frontend command and no capability entry: the plugin exposes nothing to the WebView, so `capabilities/default.json` stays at `core:default` (§8.4).
**Rationale:** Registration order is the whole mechanism. The reclaim we must not let a second instance run lives in the builder `setup`; plugin setups run first, in registration order, and single-instance's setup is where it exits the duplicate — so putting it first means the duplicate is gone before `reclaim_engine` is reachable. Enforcing at the process boundary (the OS lock) rather than with our own `engine.pid` is right because the file is exactly what a second instance would race: two instances booting together could both read "no engine" and both spawn. A dedicated, platform-native primitive has none of that window. Focusing the window turns the enforcement from a dead click into the behaviour users expect from a single-instance desktop app.
**Consequences:** `tauri-plugin-single-instance` joins the dependency set. ADR-016's reclaim is now sound: the case it could not distinguish cannot occur. The single-instance guarantee is available to everything downstream that assumes one writer of appdata (the models db, download state). The plugin must stay the first registered plugin — a future reorder that lets our `setup` run in a duplicate reintroduces the engine-kill; this is noted in `lib.rs` where it is easy to get wrong. macOS's own "one instance per .app" is *not* a substitute (a user can launch a second copy from a different path, or `open -n`), so the plugin carries all three platforms.
**Alternatives:** A `flock`/PID-file check in our own `setup` (rejected: it races — two simultaneous boots can both see the lock free — and it is exactly the `engine.pid` contention ADR-016's file already can't arbitrate; the OS primitive has no such window); rely on the OS to prevent a second instance (rejected: only macOS approximates it, and even there `open -n` and a copied bundle defeat it, so Windows and Linux would be unguarded); leave it unenforced and make the reclaim conservative — e.g. never kill an engine younger than some age (rejected: any heuristic that spares a second instance's engine also spares a genuine same-session leak, which is the failure ADR-016 exists to prevent); a full IPC/argv-forwarding layer (rejected: Darkroom takes no launch arguments, so there is nothing to forward beyond the window focus the plugin already gives).

### ADR-018 — Generate the frontend's IPC types from Rust

**Status:** accepted
**Context:** Every value that crosses `invoke`/`emit` has its shape defined twice: once in the Rust struct Tauri serialises, once in a TypeScript interface the frontend reads it back through. The M0 code kept the TS copies by hand — `engine.ts` said as much — and pinned only the Rust *serialize* output with tests. Nothing checked that the TS side still matched: a renamed field or a changed serde tag would compile clean on both sides and surface as a view that silently renders nothing, exactly the failure mode §8.6 forbids. At four types it was tolerable; the engine API grows from here, and the registry's "one definition, both the runtime contract and the CI gate" (schema in `registry.schema.ts`) is the standard the IPC surface was not yet held to.
**Decision:** The Rust structs are the single definition; `ts-rs` generates their TypeScript into `app/lib/generated/`, and `engine.ts` imports those rather than declaring its own. The derives sit behind a `ts` cargo feature so a plain `cargo build`/bundle never links ts-rs — the codegen tooling stays out of the shipped binary — while `cargo test --all-features` (what `make rust-test` and CI run) regenerates. CI regenerates on a clean checkout and `git diff --exit-code`s the result on the Linux runner: a Rust type changed without regenerating its mirror fails the build. The generated files are checked in (so a fresh clone type-checks with no Rust build) and excluded from prettier/eslint (ts-rs owns their layout). The serde-*JSON* tests stay: ts-rs checks a type's structure, not the wire bytes, so the tests remain the guard on tag keys and `rename_all` casing — the half ts-rs cannot see.
**Rationale:** Generation moves the contract from "two copies a reviewer must keep equal" to "one copy and a mechanical check," which is the same move the registry schema already made. ts-rs over the more invasive `tauri-specta` because the win we need is the type *shapes*, not a regenerated command layer — the four hand-written `invoke` wrappers are fine, and keeping them avoids pulling the whole command surface into codegen for three commands. Feature-gating keeps a dev-only dependency out of the release binary rather than trusting the linker to strip it. Checking the files in (rather than generating at build time) keeps the frontend build free of a Rust toolchain dependency and makes the contract reviewable in a diff. The one place ts-rs and Tauri disagree — `u64` maps to `bigint` in ts-rs's model but Tauri's `serde_json` IPC delivers a JSON `number` — is pinned per-field with `#[ts(type = "number")]`, because the byte counts involved are far below `2^53`.
**Consequences:** `ts-rs` joins the dependency set, optional and behind the `ts` feature. A new IPC type must derive `TS` and be regenerated, or CI's drift check fails — the cost that buys the guarantee. `app/lib/generated/` is machine-owned: edits there are overwritten. The export directory is set once via `native/.cargo/config.toml` (`TS_RS_EXPORT_DIR`, `relative = true`) because ts-rs refuses to let a per-type `export_to` escape the crate into its sibling `app/`; that config file is now load-bearing for generation landing in the right place. `make bindings` regenerates on demand; `make bindings-check` mirrors CI's gate.
**Alternatives:** Keep hand-written mirrors (rejected: the silent-drift failure this ADR exists to close, and the registry already rejected the same shape for manifests); `tauri-specta` generating the whole command layer (rejected: more churn and a heavier dependency than three commands and four wrappers justify — it can be revisited if the command surface grows large); a runtime schema check that validates payloads on arrival (rejected: it catches drift at runtime, in the user's hands, not at build time where a type mismatch belongs; and it adds per-message cost to a high-frequency channel); generate at build time rather than checking files in (rejected: it couples the frontend build to a Rust toolchain and hides the contract from review); leave `u64` as `bigint` (rejected: it is simply wrong for the wire — the runtime value is a `number` — and would force every consumer to convert a type the IPC never sends).

### ADR-019 — Decorationless window with a custom titlebar; design tokens as CSS custom properties

**Status:** accepted
**Context:** M2's design (the "Studio" direction, `docs/Darkroom Studio.dc.html`) is a single dark surface with its own 44px titlebar — a safelight status dot, the wordmark, a mono subline, and min/max/close controls whose hover colours are part of the spec (neutral `#1c1c22`, close `#c23a2a`). A native OS titlebar can't carry that; it would sit above the design as a foreign grey bar on each of the three platforms. Separately, that mockup is built from a fixed palette — ~30 hexes across five surface levels, five hairlines, seven text tones, the safelight accent and its gradient, and the error/success states — and #52 is explicitly "the exact values the rest of M2 is built from." Every later screen (#2–#9) references them, so where they live and how they're consumed is decided once here. The fonts (Space Grotesk, JetBrains Mono) are a webfont in the mockup, but the app is offline-first (Q1) and its CSP is `style-src 'self'` with no `font-src`, so a CDN fetch would be blocked silently.
**Decision:** The main window runs `decorations: false`; a React `TitleBar` component draws the chrome, using `@tauri-apps/api/window` for the controls and the native `data-tauri-drag-region` attribute for the drag surface (which adds five `core:window:allow-*` permissions to the default capability — the first grants beyond `core:default`). Tokens are Tailwind v4 `@theme` custom properties in `app/theme.css`, so each `--color-*` is both a utility class and a raw `var()` for the mockup's inline gradients — one definition, both uses, the stance the registry schema already takes. The two typefaces are self-hosted `latin`-subset woff2 under `app/fonts/`, bundled by Vite; `.mono` is the class the mockup uses for every numeric/technical value.
**Rationale:** Owning the titlebar is the only way the design renders identically on Windows, macOS, and Linux — the native bars differ in height, controls, and side. Doing it as one small chrome component keeps the platform-specific window calls in a single place rather than scattered. Tokens as CSS variables (not a TS constants module) means the value a component reads is the same value the browser paints and the same one a reviewer diffs, and it matches how `--color-surface`/`--color-safelight` already shipped in `index.css`. Self-hosting the fonts is forced by the CSP and by offline-first — and it's free: the two families total ~120 KB of `latin` woff2, far cheaper than a runtime fetch that could fail.
**Consequences:** The window is borderless, so resize grips and snap are the app's responsibility on Windows/Linux where `decorations: false` removes them — deferred, not solved here, and the reason `resizable: true` alone isn't the whole story. The default menu (`Menu::default`, ADR-015) renders as an in-window Edit/Window/Help bar on Windows and Linux, which would sit as a foreign strip below the custom titlebar; `setup` calls `window.hide_menu()` to remove it there. That call is a documented no-op on macOS, whose menu is app-wide and lives in the system bar — so Help → Open Logs stays reachable on the one platform where the menu doesn't intrude, and the M2 UI (the error banner's "View engine log", #7) becomes the cross-platform entry point. `core:window` permissions are now granted; a control that calls an ungranted method fails silently, so the capability list and the `TitleBar` calls must stay in step. The mockup's window radius (14) and drop shadow are presentation of the mockup-as-floating-window and are *not* reproduced: rounding and shadowing the app's own OS window needs `transparent: true`, which costs the native shadow and reliable edge-resize on Linux — a bad trade for a foundations issue, revisit only if the design demands a true floating window. The subline is left empty until screen routing (#2) supplies real content rather than faking the mockup's "128 outputs".
**Alternatives:** Keep native decorations and drop the custom titlebar (rejected: the design's chrome is load-bearing and can't live in an OS bar, and it would look different on each platform); a transparent rounded window with an app-drawn shadow (rejected here: loses native edge-resize and shadow on Linux for a cosmetic frame — a later call, not a foundation); tokens as a TypeScript constants module (rejected: two representations of one palette, the exact duplicate-definition trap ADR-018 and the registry schema close); ship the fonts from Google's CDN (rejected: blocked by the CSP, breaks offline, and fails silently — the failure mode §8.6 forbids); use system fonts (rejected: the design is specified in these two families and drifts visibly without them).
